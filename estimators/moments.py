...
def TSM_moms_meas(ts_meas,measured_bt):
    """
    ...
    ts_meas = Times of measurement of concentrations
    measured_bt = Measured concentrations (same length as ts_meas)
    :returns: Array of first 4 measured kth central moments to the power 1/k, such that they all have the same dimension [T].
    """
    mu_i = [np.trapz(measured_bt*ts_meas**j,x=ts_meas)/np.trapz(measured_bt,x=ts_meas) for j in range(5)]
    m_i = [mu_i[1],
           mu_i[2]-mu_i[1]**2,
...
def TSM(ts_meas,measured_bt,x,bound_cond,params0,bounds):
    """
    ...
    res = lambda params: TSM_moms_analyt(params,x,bound_cond)-TSM_moms_meas(ts_meas,measured_bt)
